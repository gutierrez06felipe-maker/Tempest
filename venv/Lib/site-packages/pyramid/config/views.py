...
@view_config(route_name='subjects', renderer='templates/subjects.jinja2')
def subjects_view(request):
    session = request.registry.db_session()
    subjects = session.query(Subject).all()
    return {"subjects": subjects}

# Vista para agregar materias
@view_config(route_name='add_subject', renderer='templates/add_subject.jinja2', request_method='POST')
...
# Vista para agregar contactos
@view_config(route_name='add_contact', renderer='templates/add_contact.jinja2', request_method='POST')
def add_contact(request):
    session = request.registry.db_session()
    name = request.params.get('name')
    email = request.params.get('email')
    subject_id = request.params.get('subject_id')
    if name and email and subject_id:
        new_contact = Contact(name=name, email=email, subject_id=subject_id)
        session.add(new_contact)
        session.commit()
    return {"message": "Contacto agregado correctamente"}
@view_config(route_name='contacts', renderer='templates/contacts.jinja2')
def contacts_view(request):
    session = request.registry.db_session()
    contacts = session.query(Contact).all()
    # gracias a la relación definida en el modelo, cada contacto tiene acceso a contact.subject.name
    return {"contacts": contacts}